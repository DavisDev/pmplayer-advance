...
static int32_t parse_ebml_trackentry(mkvinfo_t* info) {
	...
	track = malloc(sizeof(mkvinfo_track_t));
		...
		switch (ebml_read_id(info->handle, &il)) {
			...
			case MATROSKA_ID_CODECPRIVATE: {
				...
				if (num > SIZE_MAX - 1000) 
					goto err_out;
				l = x + num;
				track->private_data = malloc(num + MKV_INPUT_PADDING);
				if ( !track->private_data )
					goto err_out;
...
static int32_t parse_ebml_cues(mkvinfo_t* info) {
	...
	info->parsed_cues = realloc(info->parsed_cues, (info->parsed_cues_num+1)*sizeof(int32_t));
		...
		if (time != EBML_UINT_INVALID && track != EBML_UINT_INVALID
			&& pos != EBML_UINT_INVALID) {
				...
				if (!(info->total_indexes & 31))
					info->indexes = realloc(info->indexes, (info->total_indexes + 32) * sizeof(mkvinfo_index_t));
...
static int32_t parse_ebml_seekhead(mkvinfo_t* info) {
	...
	info->parsed_seekhead = realloc(info->parsed_seekhead,  (info->parsed_seekhead_num+1)*sizeof(int32_t));
...
void parse_ebml(mkvinfo_t* info) {
	...
	info->parsed_cues = malloc(sizeof(int32_t));
	info->parsed_seekhead = malloc(sizeof(int32_t));
