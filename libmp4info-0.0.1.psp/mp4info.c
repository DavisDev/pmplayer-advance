...
mp4info_t* mp4info_open(const char* filename) {
	...
	info->handle = sceIoOpen(filename, PSP_O_RDONLY, 0777);
	if ( !info->handle ) {
		free(info);
		return 0;
	}
	parse_atoms(info);
	sceIoClose(info->handle);
	info->handle = 0;
	return info;
}
...
void mp4info_dump(mp4info_t* info, const char* dumpfile) {
	...
	fprintf(fp, "time_scale : %d(0x%08X)\n", info->time_scale, info->time_scale);
	...
	for(i = 0; i < info->total_tracks; i++) {
		...
		fprintf(fp, "\tstts_entry_count : %d(0x%08X)\n", info->tracks[i]->stts_entry_count, info->tracks[i]->stts_entry_count);
		fprintf(fp, "\tctts_entry_count : %d(0x%08X)\n", info->tracks[i]->ctts_entry_count, info->tracks[i]->ctts_entry_count);
		fprintf(fp, "\tstss_entry_count : %d(0x%08X)\n", info->tracks[i]->stss_entry_count, info->tracks[i]->stss_entry_count);
		fprintf(fp, "\tstsc_entry_count : %d(0x%08X)\n", info->tracks[i]->stsc_entry_count, info->tracks[i]->stsc_entry_count);
		fprintf(fp, "\tstsz_entry_count : %d(0x%08X)\n", info->tracks[i]->stsz_entry_count, info->tracks[i]->stsz_entry_count);
		fprintf(fp, "\tstco_entry_count : %d(0x%08X)\n", info->tracks[i]->stco_entry_count, info->tracks[i]->stco_entry_count);
