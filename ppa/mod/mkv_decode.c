...
static int in_mkv_timestamp_queue(int* queue, unsigned int* queue_size, unsigned int queue_max, int timestamp) {
...
static int out_mkv_timestamp_queue(int* queue, unsigned int* queue_size, unsigned int queue_max, int* timestamp) {
	if ( *queue_size == 0 )
		return 0;
	*timestamp  = queue[0];
	int i;
...
static void clear_mkv_timestamp_queue(int* queue, unsigned int* queue_size, unsigned int queue_max) {
	int i;
...
char *mkv_decode_open(struct mkv_decode_struct *p, char *s, int pspType, int tvAspectRatio, int tvWidth, int tvHeight, int videoMode) {
	...
	if ( p->video_format == 0x61766331 /*avc1*/ ) {
		...
		else if ( p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data[1] == 0x42 ) {
			me_boot_start(4);
		}
		else {
			me_boot_start(3);
	...
	if ( p->video_format == 0x61766331 /*avc1*/ ) {
		int avc_profile = p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data[1];
		int avc_nal_prefix_size = ( p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data[4] & 0x03 ) + 1;
		int avc_sps_size = p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data[6];
		avc_sps_size = (avc_sps_size << 8) | p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data[7];
		int avc_pps_size = p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data[9+avc_sps_size];
		avc_pps_size = (avc_pps_size << 8) | p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data[10+avc_sps_size];
		
		result = mp4_avc_open(&p->avc, 
			avc_profile,
			((p->reader.file.video_width > 480 || p->reader.file.video_height > 272) ? 5 : 4), 
			p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data+8, 
			avc_sps_size,
			p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data+8+avc_sps_size+3, 
			avc_pps_size,
			avc_nal_prefix_size);
	}
	else {
		result = "mp4v_open: no supported";
//		result = mp4v_open_ex(&p->mp4v,
//			p->reader.file.info->tracks[p->reader.file.video_track_id]->private_data,
//			p->reader.file.info->tracks[p->reader.file.video_track_id]->private_size,
//			p->reader.file.maximum_video_sample_size);
	...
	aspect_ratio_struct_init(p->reader.file.video_width, p->reader.file.video_height, p->reader.file.display_width, p->reader.file.display_height, pspType, tvAspectRatio, tvWidth, tvHeight, videoMode);
	...
	if (p->audio_decoder == -1) {
		mkv_decode_close(p, pspType);
		return("mkv_decode_open: can not find audio decoder");
	}
	//*/
	...
	if (m33IsTVOutSupported(pspType)) {
		...
		if (p->number_of_frame_buffers < (mkv_number_of_free_video_frame_buffers + 4)) {
			mkv_decode_close(p, pspType);
			return("mkv_decode_open: number_of_frame_buffers < 4");
		}
	
	
		p->number_of_frame_buffers -= mkv_number_of_free_video_frame_buffers;
	
		i = 0;
		for (; i < mkv_number_of_free_video_frame_buffers; i++) {
...
char *mkv_decode_get_audio(struct mkv_decode_struct *p, unsigned int audio_stream, int audio_channel, int decode_audio, unsigned int volume_boost) {
	char *result;
	struct mkv_read_output_struct a_packet;
	memset(&a_packet, 0, sizeof(struct mkv_read_output_struct));
	
	result = mkv_read_get_audio(&p->reader, audio_stream, &a_packet);
	if (result != 0) {
...
char *mkv_decode_get_cached_video(struct mkv_decode_struct *p, unsigned int pic_num, unsigned int audio_stream, unsigned int volume_boost, unsigned int aspect_ratio, unsigned int zoom, unsigned int luminosity_boost, unsigned int show_interface, unsigned int show_subtitle, unsigned int subtitle_format, unsigned int loop) {
	...
	int timestamp;
	out_mkv_timestamp_queue(p->timestamp_queue, &p->timestamp_queue_size, mkv_timestamp_queue_max, &timestamp);
	double tmp ;
	tmp = 1.0f*timestamp;
...
char *mkv_decode_get_video(struct mkv_decode_struct *p, unsigned int audio_stream, unsigned int volume_boost, unsigned int aspect_ratio, unsigned int zoom, unsigned int luminosity_boost, unsigned int show_interface, unsigned int show_subtitle, unsigned int subtitle_format, unsigned int loop, int* pic_num) {
	...
	if (p->is_eof) {
		*pic_num = 1;
		in_mkv_timestamp_queue(p->timestamp_queue, &p->timestamp_queue_size, mkv_timestamp_queue_max, p->last_video_timestamp + p->video_frame_duration);
	}
	else {
		...
		if (result != 0) {
			...
			in_mkv_timestamp_queue(p->timestamp_queue, &p->timestamp_queue_size, mkv_timestamp_queue_max, v_packet.timestamp);
