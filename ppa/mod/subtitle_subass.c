...
struct subtitle_frame_struct* subtitle_parse_subass( FILE *f, char* charset, unsigned int rate, unsigned int scale ) {
	if (!f) return(0);
    ...
    while (1){
		...
		if (sscanf(line, "Dialogue: %*d,%d:%d:%d%*[.:]%d,%d:%d:%d%*[.:]%d",&a1,&a2,&a3,&a4,&b1,&b2,&b3,&b4) >= 8)
			break;
	}
