...
void flv1_play_safe_constructor(struct flv1_play_struct *p) {
	...
	flv1_decode_safe_constructor(&p->decoder);
}
...
void flv1_play_close(struct flv1_play_struct *p, int usePos, int pspType) {
	...
	flv1_decode_close(&p->decoder, pspType);

	int i = 0;
	for (i=0; i<p->subtitle_count; i++)
		subtitle_parse_close( &subtitle_parser[i] );
	
	//if (usePos) flv1_stat_save( p );
	flv1_stat_save( p );
	
	flv1_play_safe_constructor(p);
}
	

static int flv1_wait(volatile struct flv1_play_struct *p, SceUID s, char *e) {
	SceUInt t = 1;
...
static int flv1_avsync_status(int audio_timestamp, int video_timestamp, int video_frame_duration) {
...
static int flv1_show_thread(SceSize input_length, void *input) {
	volatile struct flv1_play_struct *p = *((void **) input);
	...
	while (p->return_request == 0) {
		...
		wait = flv1_wait(p, p->semaphore_can_get_video, "flv1_show_thread: sceKernelWaitSema failed on semaphore_can_get_video");
		if ( wait == -1) {
			break;
		}
		else if ( wait == 1 ) {
			
			avsync_status = flv1_avsync_status( p->decoder.output_audio_frame_buffers[p->current_audio_buffer_number].timestamp,
			...
			if(avsync_status > 0) {
				...
				if ( first == 1 ) {
					...
					if (sceKernelSignalSema(p->semaphore_can_put_video, 1) < 0) {
						p->return_result  = "flv1_show_thread: sceKernelSignalSema failed on semaphore_can_put_video";
						p->return_request = 1;
						break;
					}		
				}		
				...
				if (sceKernelSignalSema(p->semaphore_can_get_video, 1) < 0) {
					p->return_result  = "flv1_show_thread: sceKernelSignalSema failed on semaphore_can_get_video1";
					p->return_request = 1;
					break;
				}
			}
...
static void flv1_input(volatile struct flv1_play_struct *p, SceCtrlData *previous_controller) {
	scePowerTick(0);
...
static int flv1_output_thread(SceSize input_length, void *input) {
	volatile struct flv1_play_struct *p = *((void **) input);
	...
	while (p->return_request == 0) {
		volatile struct flv1_decode_buffer_struct *current_buffer = &p->decoder.output_audio_frame_buffers[p->current_audio_buffer_number];
		
		wait = flv1_wait(p, p->semaphore_can_get_audio, "flv1_output_thread: sceKernelWaitSema failed on semaphore_can_get_audio");
		if ( wait == -1) {
			break;
		}
		else if ( wait == 1 ) {
			...
			if (first == 1) {
				...
				if (sceKernelSignalSema(p->semaphore_can_put_audio, 1) < 0) {
					p->return_result  = "flv1_output_thread: sceKernelSignalSema failed on semaphore_can_put_audio";
					p->return_request = 1;
					break;
				}
			}
...
void flv1_play_do_seek(volatile struct flv1_play_struct *p) {
	...
	if (p->resume_pos>0) {
		...
		p->resume_pos = 0;
		flv1_decode_seek((struct flv1_decode_struct *) &p->decoder, pos, pos);
		return;
	}
...
void flv1_play_reset(volatile struct flv1_play_struct *p) {
	int i;
	for(i = 0; i < p->decoder.number_of_frame_buffers; i++)
		p->decoder.output_video_frame_buffers[i].timestamp = -p->decoder.video_frame_duration;
	flv1_decode_reset((struct flv1_decode_struct *) &p->decoder);
}

static int flv1_demux_thread(SceSize input_length, void *input) {
	volatile struct flv1_play_struct *p = *((void **) input);
	int cached_video_frame = 0;
	
	while(p->return_request == 0 && !flv1_decode_is_eof((struct flv1_decode_struct *) &p->decoder)) {
		int wait;
		char* result;
		
		wait = flv1_wait(p, p->semaphore_can_put_audio, "flv1_play_start: sceKernelWaitSema failed on semaphore_can_put_audio");
		if ( wait == -1) {
			p->return_request = 1;
			break;
		}
		else if ( wait == 1) {
			result = flv1_decode_get_audio((struct flv1_decode_struct *) &p->decoder, p->audio_stream, p->audio_channel, 1, p->volume_boost);
			if (result != 0) {
			...
			if (sceKernelSignalSema(p->semaphore_can_get_audio, 1) < 0) {
				p->return_result  = "flv1_play_start: sceKernelSignalSema failed on semaphore_can_get_audio";
				p->return_request = 1;
				break;
			}
		}
		
		wait = flv1_wait(p, p->semaphore_can_put_video, "flv1_play_start: sceKernelWaitSema failed on semaphore_can_put_video");
		if ( wait == -1) {
			p->return_request = 1;
			break;
		}
			...
			if ( cached_video_frame > 0 ) {
				result = flv1_decode_get_cached_video((struct flv1_decode_struct *) &p->decoder, cached_video_frame, p->audio_stream, p->volume_boost, p->aspect_ratio, p->zoom, p->luminosity_boost, p->show_interface, p->subtitle, p->subtitle_format, p->loop);
				if (result != 0) {
					p->return_result  = result;
					p->return_request = 1;
					break;
				...
				if (sceKernelSignalSema(p->semaphore_can_get_video, 1) < 0) {
					p->return_result  = "flv1_play_start: sceKernelSignalSema failed on semaphore_can_get_video";
					p->return_request = 1;
					break;
				}
			}
			else {
				result = flv1_decode_get_video((struct flv1_decode_struct *) &p->decoder, p->audio_stream, p->volume_boost, p->aspect_ratio, p->zoom, p->luminosity_boost, p->show_interface, p->subtitle, p->subtitle_format, p->loop, &cached_video_frame);
				if (result != 0) {
					p->return_result  = result;
					p->return_request = 1;
					break;
				...
				if ( cached_video_frame >= 0 ) {
					...
					if (sceKernelSignalSema(p->semaphore_can_put_video, 1) < 0) {
						p->return_result  = "flv1_play_start: sceKernelSignalSema failed on semaphore_can_put_video1";
						p->return_request = 1;
						break;
					}
				}
		...
		if ( flv1_decode_is_eof((struct flv1_decode_struct *) &p->decoder) ) {
...
char *flv1_play_start(volatile struct flv1_play_struct *p) {
	...
	while (p->return_request == 0 && !flv1_decode_is_eof((struct flv1_decode_struct *) &p->decoder)) {
		...
		flv1_input(p, &previous_controller);
...
char *flv1_play_open(struct flv1_play_struct *p, struct movie_file_struct *movie, int usePos, int pspType, int tvAspectRatio, int tvWidth, int tvHeight, int videoMode) {
	...
	char *result = flv1_decode_open(&p->decoder, movie->movie_file, pspType, tvAspectRatio, tvWidth, tvHeight, videoMode);
	if (result != 0) {
	...
	if (p->semaphore_can_get_video < 0) {
		flv1_play_close(p, 0, pspType);
		return("flv1_play_open: sceKernelCreateSema failed on semaphore_can_get_video");
	}
	...
	if (p->semaphore_can_put_video < 0) {
		flv1_play_close(p, 0, pspType);
		return("flv1_play_open: sceKernelCreateSema failed on semaphore_can_put_video");
	}
	...
	if (p->semaphore_can_get_audio < 0) {
		flv1_play_close(p, 0, pspType);
		return("flv1_play_open: sceKernelCreateSema failed on semaphore_can_get_audio");
	}
	...
	if (p->semaphore_can_put_audio < 0) {
		flv1_play_close(p, 0, pspType);
		return("flv1_play_open: sceKernelCreateSema failed on semaphore_can_put_audio");
	}


	//p->output_thread = sceKernelCreateThread("output", flv1_output_thread, 0x8, 0x10000, 0, 0);
	p->output_thread = sceKernelCreateThread("output", flv1_output_thread, 0x3D, 0x10000, PSP_THREAD_ATTR_USER|PSP_THREAD_ATTR_VFPU, 0);
	if (p->output_thread < 0) {
		flv1_play_close(p, 0, pspType);
		return("flv1_play_open: sceKernelCreateThread failed on output_thread");
	}

	//p->show_thread = sceKernelCreateThread("show", flv1_show_thread, 0x8, 0x10000, 0, 0);
	p->show_thread = sceKernelCreateThread("show", flv1_show_thread, 0x3F, 0x10000, PSP_THREAD_ATTR_USER|PSP_THREAD_ATTR_VFPU, 0);
	if (p->show_thread < 0) {
		flv1_play_close(p, 0, pspType);
