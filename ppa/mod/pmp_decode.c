...
char *pmp_decode_open(struct pmp_decode_struct *p, char *s)
	...
	if ( p->reader.file.header.audio.format == 1 )
		{
		p->audio_context->sample_rate = p->reader.file.header.audio.rate;
		}
	
	if ( p->reader.file.header.audio.format == 2 || p->reader.file.header.audio.format == 21 || p->reader.file.header.audio.format == 22 ) 
		{
		if ( p->reader.file.header.audio.format == 2 )
			p->audio_context->block_align = 384;
		else if (p->reader.file.header.audio.format == 21)
			p->audio_context->block_align = 304;
		else if (p->reader.file.header.audio.format == 22)
			p->audio_context->block_align = 192;
		}
	//*/
